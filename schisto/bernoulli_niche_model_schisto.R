...
#only relevant if running sans qsub and with mclapply
#library('parallel')
#slots = 40  
...
## Set data location
data_loc <- (paste0(j, '/temp/stearns7/eco_niche/schisto_data'))

# Set output path
outpath <- (paste0(j, '/temp/stearns7/eco_niche/output'))
...
buffer <- raster(paste0(data_loc, "/covariates/man_reclass_prj.tif"))
print('Loading buffer grid for background point generation')

bg <- bgSample(buffer, # Weighting grid - population in this case, custom function defined in github 
               n = 2500, # Background data points desired; up to 10,000 point in final run
               prob = TRUE, # Set to FALSE so doesn't weight by raster specified above
               replace = TRUE,
               spatial = FALSE)
...
write.csv(dat_all, file = (paste0(outpath, "/interim_output/dat_all", run_date, ".csv"))) ## may be able to remove if do not run the below
...
#3. combinePreds
########################################################################################
#performed in parallel: get model stats
